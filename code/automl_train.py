...
def download_data():
    ...
    # consider replacing errors.csv with anoms.csv (results from running anomaly detection)
    # urllib.request.urlretrieve(container + 'errors.csv', filename='../data/errors.csv')
    urllib.request.urlretrieve(container + 'anoms.csv', filename='../data/errors.csv')
...
#                              X = X_train.values, # we convert from pandas to numpy arrays using .values
#                              y = y_train.values[:, 0], # we convert from pandas to numpy arrays using .values
#                              path=project_folder, )
...
# print("AutoML execution completed")
